...
2. Chaque page répète `require autoload.php`, `session_start()`, `require db.php`, `require render.php`.
...
│   └── .htaccess                 pour Apache ; inutile avec php -S
...
connaît pas ce chemin et répond 404.
...
- **Oublier `-t public`.** Le serveur sert alors la racine du projet, et tout redevient accessible. Le
  script `verifier.sh` teste explicitement `/../database.sqlite`.
...
./verifier.sh 07                        # vérifie aussi le 404 et l'inaccessibilité de la base
```
