...
Les closures évitent de construire tous les contrôleurs à chaque requête : seul celui de la route trouvée est
instancié. `$controleur->$action()` appelle la méthode dont le nom est dans `$action` : c'est ce qui permet à
...
HTTP réelle : `exit` arrêterait le test. Étape 9.
