...
2. **La portée de la fonction** est la protection. Le `require` a lieu à l'intérieur de `render()`, donc la vue
   ne voit que les variables locales de `render()` : celles issues d'`extract()`, plus `$vue` et `$donnees`. `$pdo`,
...
3. **`utilisateurConnecte`** est ajouté à toutes les vues par `render()` : la navigation en a besoin partout,
   et c'est la seule lecture de `$_SESSION` autorisée hors des pages. `$donnees += [...]` est l'union de
   tableaux : elle n'ajoute que les clés **absentes**, une page qui passerait déjà `utilisateurConnecte`
   garderait sa valeur. `$_SESSION['utilisateur'] ?? null` vaut la clé si elle existe, `null` sinon, sans
   avertissement.

`EXTR_SKIP` empêche une clé du tableau d'écraser une variable existante. Ici les seules variables en jeu sont
`$vue` et `$donnees`, mais la précaution devient importante quand les données viennent de l'utilisateur.
...
- **`header('Location')` après `render()`.** Une fois du HTML envoyé, PHP ne peut plus ajouter d'en-tête :
  `Cannot modify header information - headers already sent`. Piège d'autant plus vicieux qu'avec `php -S`
  et le `php.ini` de développement, la sortie est mise en tampon (`output_buffering = 4096`) : sur une petite
  page, la redirection semble marcher, puis échoue en production ou dès que la page dépasse 4 Ko. Toujours
...
./verifier.sh 04               # depuis la racine du dépôt
```
