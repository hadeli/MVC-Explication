...
Soit une vingtaine de lignes dupliquées par page, sur des fichiers d'une soixantaine de lignes.
...
On utilise `__DIR__` plutôt qu'un chemin relatif : le chemin relatif dépend du dossier courant du processus,
pas du fichier en cours, ce qui casse dès qu'on lance le serveur depuis un autre dossier.
...
- **Un `<?php` ou `?>` qui déborde.** Un retour à la ligne après `?>` à la fin de `db.php` serait envoyé au
  navigateur avant le HTML et empêcherait les redirections. Ici `db.php` n'a pas de balise fermante, c'est voulu.
...
- **Une variable oubliée.** Si une page oublie `$titre`, `header.php` produit un avertissement. Le contrat entre
  la page et l'include n'est écrit nulle part.
