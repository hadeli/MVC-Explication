...
À l'étape 10, ajouter une fonctionnalité demande de toucher **trois** endroits : écrire la méthode dans un
...
│   │   └── ... (Request, Response, View, Template, Env, Database inchangés)
...
l'ancienne méthode, au caractère près.
...
`ControllerInterface`. Puis `$classe::support($request)` : PHP appelle la méthode statique d'une classe dont
le nom est dans une variable. C'est l'autoloader de l'étape 6 qui charge le fichier à ce moment-là.
...
final class LoginController extends AbstractController
{
    ...
    public function handle(Request $request): Response
    {
        ...
        /* ... exactement l'ancien AuthController::login() ... */
...
`LoginFormController` ne reçoit que la vue. À l'étape 10, `AuthController` recevait tout pour tout le monde.
...
C'est le bénéfice concret d'une interface : on peut fabriquer un faux contrôleur en trois lignes.
...
| Un seul contrôleur construit par requête, avec ses seules dépendances | Huit fichiers (six contrôleurs, une abstraite, un trait) au lieu de deux |
...
| Un faux contrôleur pour tester le routeur tient en quatre lignes | |
...
(`RequestHandlerInterface` et `MiddlewareInterface` de la PSR-15 ont exactement la forme de `handle()`).
...
déclare déjà. Une classe correcte mais absente de la liste ne répond jamais, sans erreur. Étape 12.
