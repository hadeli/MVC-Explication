...
Deux écritures récentes : `?? throw new …` lève l'exception quand la valeur de gauche est `null` (PHP 8), et
`str_starts_with()` remplace un `substr() === 'sqlite:'` (PHP 8). Dans `env.php`, `[$cle, $valeur] = explode('=', $ligne, 2)`
affecte les deux morceaux d'un coup, et `$valeur[-1]` est le dernier caractère.

Un chemin SQLite relatif est résolu depuis la racine du projet, pas depuis le dossier courant : sans cela,
la base serait créée dans le dossier courant du serveur. Avec `php -S`, c'est le dossier passé à `-t` : ici le
dossier de l'étape, sans conséquence, mais `public/` à partir de l'étape 7, où la base redeviendrait
téléchargeable. `verifier.sh` le signale.
...
Tests manuels utiles : supprimer `.env` et recharger : le navigateur affiche une page vide (erreur 500), le message est dans le terminal où tourne `php -S`. Il s'affiche aussi dans la page si `display_errors` est activé dans `php.ini`. Changer `DB_DSN` vers un autre fichier,
